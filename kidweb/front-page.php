 ...
 * The homepage template for KIDazzle Child Care
 * @package KIDWEB
 */
get_header();
?>
...
<section class="py-20 bg-slate-50 relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10">
        ...
        <div class="grid md:grid-cols-3 gap-6 lg:gap-10">
            ...
            <!-- Doral -->
            <a href="<?php echo esc_url(home_url('/location-miami/')); ?>"
                class="group relative rounded-[2.5rem] overflow-hidden h-96 shadow-lg cursor-pointer transform hover:-translate-y-2 transition duration-300">
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="grid md:grid-cols-4 gap-8">
            <a href="<?php echo esc_url(home_url('/meals/')); ?>"
                ...
                <span
                    class="text-orange-600 font-bold text-xs flex items-center justify-center gap-1 uppercase tracking-wide">Learn
            ...
            <a href="<?php echo esc_url(home_url('/staff/')); ?>"
                ...
                <p class="text-sm text-slate-600 mb-4 leading-relaxed">Low turnover and tenured educators who truly know
                    your child.</p>
                <span
                    class="text-red-600 font-bold text-xs flex items-center justify-center gap-1 uppercase tracking-wide">Learn
                    More <i data-lucide="arrow-right" class="w-3 h-3"></i></span>
            </a>
            <a href="<?php echo esc_url(home_url('/curriculum/')); ?>"
                ...
                <p class="text-sm text-slate-600 mb-4 leading-relaxed">Research-based curriculum tailored to each
                    developmental stage.</p>
                <span
                    class="text-cyan-600 font-bold text-xs flex items-center justify-center gap-1 uppercase tracking-wide">Learn
                    More <i data-lucide="arrow-right" class="w-3 h-3"></i></span>
            </a>
            <a href="<?php echo esc_url(home_url('/safety/')); ?>"
                ...
                <p class="text-sm text-slate-600 mb-4 leading-relaxed">Secure keypad entry and monitored surveillance
                    exceeding state standards.</p>
                <span
                    class="text-green-600 font-bold text-xs flex items-center justify-center gap-1 uppercase tracking-wide">Learn
                    More <i data-lucide="arrow-right" class="w-3 h-3"></i></span>
            </a>
        </div>
    </div>
        ...
        <div class="relative z-10">
            ...
            <div
                ...
                <iframe src="https://api.leadconnectorhq.com/widget/form/N8RYaUY1SuORexcyA6la"
                    ...
                    data-layout="{'id':'INLINE'}" data-trigger-type="alwaysShow" data-activation-type="alwaysActivated"
                    data-deactivation-type="neverDeactivate" data-form-name="2023 New KIDazzel website contact"
                    data-height="870" data-form-id="N8RYaUY1SuORexcyA6la" title="Contact Form"></iframe>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.renderGraph = function (stage) {
            ...
            document.getElementById('stage-buttons').innerHTML = Object.keys(growthData).map(k => `<button onclick="renderGraph('${k}')" class="px-4 py-2 rounded-full border border-slate-200 hover:bg-slate-100 ${k === stage ? 'bg-slate-900 text-white hover:bg-slate-800' : ''}">${growthData[k].label}</button>`).join('');
            ...
            document.getElementById('graph-bars').innerHTML = data.stats.map(s => `<div class="w-full mx-1 relative group h-full flex items-end"><div class="w-full rounded-t-lg ${s.col} graph-bar relative" style="height:${s.val}%"><div class="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition">${s.val}%</div></div></div>`).join('');
            document.getElementById('graph-labels').innerHTML = data.stats.map(s => `<div class="text-center text-xs font-bold text-slate-500 w-full">${s.lbl}</div>`).join('');
        }
        renderGraph('infants');
    });
</script>
