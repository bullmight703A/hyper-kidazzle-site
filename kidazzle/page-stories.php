  ...
  <header class="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-kidazzle-blue/10">
    <div class="max-w-7xl mx-auto px-4 lg:px-6 h-[82px] flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-8 text-sm font-semibold text-brand-ink/80">
        ...
        <a href="<?php the_permalink(); ?>" class="text-brand-ink font-bold hover:text-kidazzle-red">Stories</a>
        <?php
        $newsroom_page = get_page_by_path('newsroom');
        if ($newsroom_page):
          ?>
          <a href="<?php echo esc_url(get_permalink($newsroom_page->ID)); ?>"
            class="hover:text-kidazzle-yellow">Newsroom</a>
        <?php endif; ?>
      </nav>
      <?php
          ...
          <a href="<?php echo esc_url(get_permalink($featured_post_id)); ?>" class="block">
            <div class="relative rounded-[3rem] overflow-hidden shadow-soft group cursor-pointer h-[500px]">
              ...
              <div class="absolute bottom-0 left-0 p-8 md:p-12">
                <span
                  class="bg-kidazzle-yellow text-brand-ink text-[10px] font-bold uppercase px-3 py-1 rounded-full mb-4 inline-block">Featured</span>
