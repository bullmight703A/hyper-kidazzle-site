  ...
  <header class="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-kidazzle-blue/10">
    <div class="max-w-7xl mx-auto px-4 lg:px-6 h-[82px] flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-8 text-sm font-semibold text-brand-ink/80">
        ...
        ?>
        <a href="<?php echo esc_url($stories_url); ?>" class="hover:text-kidazzle-blue flex items-center gap-2"><i
  ...
  <main>
    <article>
      <header class="py-20 text-center max-w-4xl mx-auto px-4">
        <div class="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-kidazzle-blue mb-6">
          <span class="w-2 h-2 bg-kidazzle-blue rounded-full"></span> <?php echo esc_html($primary_category); ?>
      ...
      <div class="max-w-3xl mx-auto px-4 lg:px-6 pb-20">
        <div
          class="post-content prose prose-lg prose-headings:font-serif prose-headings:font-bold prose-p:text-brand-ink/90 prose-a:text-kidazzle-blue hover:prose-a:text-kidazzle-blue/80 transition-colors">
          <?php
          while (have_posts()):
            the_post();
            the_content();
            ...
            <?php while ($related_query->have_posts()):
              ...
              <a href="<?php the_permalink(); ?>" class="group">
                <div class="rounded-2xl overflow-hidden mb-4 h-48">
                  ...
                  <?php else: ?>
                    <div class="w-full h-full bg-kidazzle-blue/10"></div>
                  <?php endif; ?>
                </div>
                <h4 class="font-bold text-lg leading-tight group-hover:text-kidazzle-blue"><?php the_title(); ?></h4>
              </a>
            <?php endwhile;
            wp_reset_postdata(); ?>
          </div>
